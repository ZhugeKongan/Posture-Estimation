...
def get_args():
    ...
    parser.add_argument("--output", "-o", type=str,
                        help="path to output database mat file", default='/disks/disk2/data/CACD2000_processed')
    parser.add_argument("--img_size", type=int, default=250,
                        help="output image size")
    parser.add_argument("-annotation", type=str,
                        help="path to .landmark files", default='/disks/disk2/lishengyan/MyProject/Posture_Estimation/dataset/cacd/landmark/')
...
def process_split_file():
    file_path = '/disks/disk2/lishengyan/MyProject/Posture_Estimation/dataset/cacd/'
...
def main():
    ...
    mypath = '/disks/disk2/data/CACD2000/'
    isPlot = False
...
#        landmark_list.append(get_landmarks(onlyfiles[i], args))
    ...
    landmark_ref = np.matrix(np.load('/disks/disk2/lishengyan/MyProject/Posture_Estimation/dataset/cacd/CACD_mean_face.npy', allow_pickle=True))
    ...
    for i in tqdm(range(len(onlyfiles))):
        ...
        input_img = warp_im(input_img, M, (250, 250, 3))
        io.imsave(args.output +'/'+ img_name, input_img)

if __name__ == '__main__':
    main()
# import glob
# train_dic = np.load('/disks/disk2/lishengyan/MyProject/Posture_Estimation/dataset/cacd/train_cacd_processed.npy', allow_pickle=True).item()
# available_imgs = glob.glob('/disks/disk2/data/CACD2000/*.jpg')
# imgs=os.listdir('/disks/disk2/data/CACD2000/')
# print(len(available_imgs))
# print(len(train_dic),len(valid_dic),len(eval_dic))
# train_imgs=[]
# for key in train_dic.keys():
#     p=train_dic[key]['path'][0].split('/')[-1].split('_')[1:-1]
#     p="_".join(p)
#     for img in imgs:
#         if p in img:
#             if(len(p.split('_'))+2==len(img.split('_'))):
#                 train_imgs.append(img)
# print(len(train_imgs))
# train_file=pd.DataFrame(columns=["file"],data=train_imgs)
